...
const { "2:36" : { fields: flagFields } } = require('../parsers/response')
const flagsTable = flagFields.reduce(function(result, item) {
  result[item.name] = item.mask
  return result
}, {})
...
module.exports = function sphero(device) {
  ...
  device.setPermOptionFlags = function(flags, callback) {
    if (typeof flags === 'object') {
      flags = Object.entries(flags).reduce((res, [key, value]) => {
                if (!value || !flagsTable[key]) return res
                res |= flagsTable[key]
                return res
              }, 0x00)
    }
